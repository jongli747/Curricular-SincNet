...
#model_file = '/home/mirco/sincnet_models/SincNet_TIMIT/model_raw.pkl'
model_file = 'exp/SincNet_TIMIT/sincnet_curricular.pkl'
# Config file of the speaker-id experiment used to generate the model
#cfg_file = '/home/mirco/SincNet/cfg/SincNet_TIMIT.cfg'
cfg_file = 'cfg/SincNet_TIMIT.cfg'
te_lst = 'data_lists/libri_te.scp'  # List of the wav files to process
# output dictiona mg the a sentence id as key as the d-vector as value
out_dict_file = 'libri_curricularface.npy'
...
cnn_use_batchnorm = list(map(str_to_bool,
                             options.cnn_use_batchnorm.split(',')))
...
with torch.no_grad():
    ...
    for i in range(snt_te):
        ...
        sig_arr = torch.zeros([Batch_dev,
                               wlen]).float().to(device).contiguous()
        dvects = Variable(
            torch.zeros(N_fr, d_vector_dim).float().to(device).contiguous())
        count_fr = 0
        count_fr_tot = 0
        while end_samp < signal.shape[0]:
            ...
            if count_fr == Batch_dev:
                ...
                # print(dvects.shape, 'dvects')
                #print((DNN1_net(CNN_net(inp))).shape, 'dnn1')
                count_fr = 0
                sig_arr = torch.zeros([Batch_dev,
                                       wlen]).float().to(device).contiguous()
        ...
        if avoid_small_en_fr:
            dvects = dvects.index_select(0,
                                         (en_arr_bin == 1).nonzero().view(-1))

        # averaging and normalizing all the d-vectors
        print(dvects.shape, 'dvects')
        d_vect_out = torch.mean(dvects / dvects.norm(p=2, dim=1).view(-1, 1),
                                dim=0)
        ...
        # dict_key = wav_lst_te[i].split('/')[-2] + '/' + wav_lst_te[i].split(
        #     '/')[-1]
